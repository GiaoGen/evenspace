...
- 当前照片和涂鸦以 `imageDataUrl` 形式进入本地 JSON；这不是生产媒体存储方案。
- 当前 Chat 会调用浏览器麦克风并生成本地音频 data URL，但没有服务端上传、转码、扫描或跨设备授权访问。
...
- Board 评论当前随 photo 本地记录；生产端需要单独授权、限流、审计和删除策略，不能只依赖前端隐藏输入框。
