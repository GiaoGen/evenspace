...
> 状态：2026-07-19。正式产品路由已经从静态高保真原型进入本地优先可操作 Mock。本文用于防止把“界面存在”“浏览器本地可操作”和“生产后端已安全实现”混为一谈。
...
- 按住录制、左滑取消、松开发送且最多 60 秒的真实浏览器本地录音；调用麦克风并生成 data URL，但不上传服务器。
...
- Board / Sequence 切换；添加真实本地照片、便签或涂鸦。
...
- 未开始/进行中/已结束由浏览器当前时间自动推导；进入页面自动定位当前或下一行程。
...
- 三键 Dock 负责 fit、背景和创建；创建卡提供 Camera / Photos / Note / Doodle，背景卡提供 Stone / Linen / Night。
...
- Chat/Board 媒体仍是 data URL 和 `localStorage` JSON；没有 Blob asset 层、服务端上传、EXIF 保证、私有访问控制和删除流水线。
