...
export function CreateRoomWizard() {
  ...
  if (session.viewer.authState !== "signed-in") {
    return <div className={styles.page}><WizardHeader title="New room" returnLabel="Return to rooms" /><main className={styles.authRequired}><span><Icon name="members" /></span><h1>Sign in to host</h1><Link href="/account">Open account <Icon name="arrow" /></Link></main></div>;
  }
  ...
  if (state.status === "complete") {
    ...
    return <div className={styles.page}><WizardHeader title="Room ready" returnLabel="Return to rooms" /><main className={styles.complete}>
      ...
      <InvitationCard name={state.room.name} draft={state.room.draft} endTime={endTime} />
      <div className={styles.inviteActions}><button type="button" onClick={() => downloadInvitationCard(state.room.name, state.room.draft, endTime)}><Icon name="image" />Save card</button>{createdPublicId ? <Link href={`/rooms/${createdPublicId}`}>Open this room <Icon name="arrow" /></Link> : null}</div>
    </main></div>;
  }
