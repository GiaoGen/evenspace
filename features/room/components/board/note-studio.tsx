...
export function NoteStudio({ onClose, onAdd }: { readonly onClose: () => void; readonly onAdd: (text: string, variant: BoardNoteVariant) => void }) {
  ...
  return <div className={styles.noteStudioBackdrop} role="dialog" aria-modal="true" aria-label="Create a text card" onPointerDown={onClose}>
    <section className={styles.noteStudio} onPointerDown={(event) => event.stopPropagation()}>
      <div className={styles.noteCardRail}>
        {variants.map((value) => <article key={value} className={`${styles.noteStyleCard} ${styles[`note${value}`]} ${variant === value ? styles.noteStyleSelected : ""}`} onPointerDown={() => setVariant(value)}>
          {variant === value
            ...
            : <button type="button" onClick={() => setVariant(value)} aria-label={`Use ${value} note style`}><span>{text.trim() || "A small thought."}</span></button>}
        </article>)}
      </div>
