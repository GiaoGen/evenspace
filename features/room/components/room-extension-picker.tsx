...
export function RoomExtensionPicker({ minutes, maxMinutes, endsAt, nowIso, onChange, onCancel, onConfirm }: { readonly mode?: "direct" | "vote"; readonly minutes: number; readonly maxMinutes: number; readonly endsAt: string | null; readonly nowIso: string; readonly onChange: (minutes: number) => void; readonly onCancel: () => void; readonly onConfirm: () => void }) {
  ...
  return <section className={styles.extensionPicker} aria-label="Choose extension duration">
    <header><span>Extend by</span><strong>+ {disabled ? "0 min" : formatExtensionDuration(minutes)}</strong></header>
    ...
    <footer><button type="button" onClick={onCancel}>Cancel</button><button type="button" className={styles.confirm} disabled={disabled} onClick={onConfirm}>Extend room</button></footer>
  </section>;
}
