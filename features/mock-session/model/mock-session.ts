...
import type { BoardItem, ChatMessage, ItineraryItem, MembershipState, PersonSummary, PollPreview, RoomCapabilities, RoomDetail, RoomSummary } from "@/core/domain/room";
...
const isMessage = (value: unknown) => isRecord(value) && isText(value.id, 100, false) && ["message", "system"].includes(String(value.kind)) && (value.author === null || isPerson(value.author)) && isText(value.body, 2000) && isIso(value.sentAt) && Array.isArray(value.reactions) && value.reactions.length <= 24 && value.reactions.every((reaction) => isRecord(reaction) && isText(reaction.emoji, 16, false) && isInt(reaction.count, 0, 100000));
...
function isMockRoom(value: unknown) {
  ...
  if (!isText(value.timeZone, 80, false) || !isIso(value.endsAt, true) || !isIso(value.archivedAt, true) || !isIso(value.createdAt) || !isInt(value.memberCount, 0, 100) || !isInt(value.memberLimit, 2, 100) || !isInt(value.photoCount, 0, 500) || !isInt(value.maxPhotos, 1, 500) || !isInt(value.mediaLimitMb, 1, 100000)) return false;
