...
### 10.2 Phase F3-T：Typography 插入阶段

F3-B3 实现暴露了“现有七个 typography role 共享 Geist Sans、CJK 由系统 fallback 决定”的全局艺术指导与 reality gap。为避免继续用单一 sans 审批 Grid/Contact，且避免 F3-B4/F3-B5 在未知字体系统上累积视觉债，插入两个严格分离的阶段：

#### F3-T1 — Typography Art Direction & Font System Decision（已批准）

- 只进行官方来源/许可/CJK/摄影编辑研究、现状审计、2–3 套完整方向、受控 preset、specimen 内容矩阵与用户裁决材料。
- 交付 [Typography Art Direction](./docs/recipe-catalog/typography-art-direction.md)、[Typography Source Dossier](./docs/recipe-catalog/typography-source-dossier.md) 与 [Typography Specimen Matrix](./docs/recipe-catalog/typography-specimen-matrix.md)。
- 禁止下载字体、改动字体文件、TS/TSX/CSS/测试/Recipe Definition/Token、启动服务器/浏览器、激活 Recipe 或进入 F3-B4。
- 当前推荐 S1 Duplex Photo-Essay，S2 Plex Unified Archive 为唯一保守 fallback；S3 Moda-First Fashion Gloss 因 CJK hard fail 淘汰。以上均等待 D01–D08 用户裁决，不构成自动批准。

#### F3-T2 — Typography Asset, Specimen & Runtime Reality（当前：等待用户视觉 Gate）

- 只有用户通过 F3-T1 Gate 后才可单独派发；固定 upstream release、license、hash、locale、weight、unicode-range 与真实 bytes，形成字体资产 manifest。
- 用获批 S1 与锁定 fallback S2 在同一内容、几何、preset metrics 下完成繁中/简中/英文/混排、最长文本、font-ready、Editor/Reader 与移动端 specimen。
- 任何 contract/token/estimator/CSS/资产变更必须在 F3-T2 任务中逐项最小化，不得把 F3-T1 文档当成实现授权。
- F3-T2 完成后先回归 Quiet、Editorial、Grid/Contact 全部 **9 个已实现 Anchor** 及既有压力点；Grid/Contact visual Gate 重新开放。只有该 Gate 通过后才恢复 F3-B4 Dynamic；F3-B5 同样保持暂停。

#### Gate F3-T2（当前停止点）

用户检查 S1/S2 关键 specimen 与 Quiet、Editorial、Grid/Contact 9 个已实现 Anchor。视觉 Gate 未通过前不恢复 F3-B4/F3-B5，不激活任何 Recipe。
