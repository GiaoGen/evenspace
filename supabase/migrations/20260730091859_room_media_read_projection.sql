create function public.list_room_card_media(requested_room_ids uuid[])
returns table(
  photo_id uuid,
  room_id uuid,
  asset_id uuid,
...
as $$
  with ranked_photos as (
    select
      ...
      row_number() over (partition by photo.room_id order by photo.created_at desc, photo.id desc) as photo_rank
...
insert into private.schema_versions(component,version) values('room_media_read_projection',1)
